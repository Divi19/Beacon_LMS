...
export default function InstructorLessonCreation() {
  ...
  useEffect(() => {
    ...
    async function checkCourses() {
      try {
        const res = await axios.get("http://localhost:8000/lessons/frontend/");
        if (!cancelled && Array.isArray(res.data) && res.data.length > 0) {
          // Instructor has at least one course — go to the list view
          navigate("/instructor/lesson-list", { replace: true });
        }
  ...
  return (
    <div className={i.wrap}>
      ...
      <header className={i.header}>
        <h1 className={i.title}>LESSONS</h1>
      </header>

      <section className={i.card}>
        <p className={i.emptyText}>
          No lessons yet. Create one<br />
        </p>
