...
export default function StudentLessonDetail() {
    ...
    async function apiGetLesson() {
        ...
        const res = await api.get(
            `/student/courses/lesson/detail/${lessonId}/`,
        );
    ...
    async function apiGetClassrooms() {
        //GET
        const res = await api.get(
            `/student/lessons/${lessonId}/classrooms/unenrolled/`,
        );
        return res.data;
    }
    ...
    async function apiEnrollClassroom(classroom_id) {
        ...
        const res = await api.post(
            `/student/lessons/${lessonId}/classrooms/enroll/${classroom_id}/`,
        );
        return res.data;
    }
    ...
    async function apiLeaveClassroom(classroom_id) {
        ...
        const res = await api.delete(
            `/student/lessons/${lessonId}/classrooms/enrolled/${classroom_id}/`,
        );
        return res.data;
    }
    ...
    async function apiEnrollLesson() {
        const res = await api.post(
            `/student/courses/${courseId}/lessons/enroll/${lessonId}/`,
        );
        return res.data;
    }
        ...
        {showPickClassList && (
          <>
            ...
            <div className={i.rect4}>
                ...
                {showPickClassList && (
                    <>
                        <div className={i.noChosenText}>
                            No Chosen Classroom yet
                        </div>
                        <div className={i.clsList}>
                            {classrooms.map(c => {
                                ...
                                const capacity = Number.isFinite(c.capacity)
                                    ? c.capacity
                                    : 0;
                                const isActive = c.is_active !== false;
                                const full = occupied >= capacity || !isActive;
                                ...
                                return (
                                    ...
                                    >
                                        <div className={i.clsColDay}>
                                            <div className={i.clsDay}>
                                                {c.day_of_week}
                                            </div>
                                            <div className={i.clsDur}>
                                                {fmtHours(c.duration_minutes)}
                                            </div>
                                        </div>
                ...
                {showChosenClass && (
                    <div className={i.clsList}>
                        <div className={i.clsCard}>
                            <div className={i.clsColDay}>
                                <div className={i.clsDay}>
                                    {chosenClassroom.day_of_week}
                                </div>
                                <div className={i.clsDur}>
                                    {fmtHours(chosenClassroom.duration_minutes)}
                                </div>
                            </div>

                            <div className={i.clsColTime}>
                                <div className={i.clsTime}>
                                    {chosenClassroom.time_start} -{" "}
                                ...
                                <div className={i.clsMetaRow}>
                                    ...
                                    <span>
                                        ID: {chosenClassroom.classroom_id}
                                    </span>
                                </div>
                            </div>
