# Generated by Django 5.2.6 on 2025-10-05 16:42

import api.models
import django.db.models.deletion
from django.db import migrations, models


class Migration(migrations.Migration):

    initial = True

    dependencies = []

    operations = [
        migrations.CreateModel(
            name="Classroom",
            fields=[
                (
                    "classroom_id",
                    models.CharField(
                        default=api.models.generate_custom_id,
                        editable=False,
                        max_length=6,
                        primary_key=True,
                        serialize=False,
                        unique=True,
                    ),
                ),
                ("director", models.CharField(blank=True, max_length=255, null=True)),
                ("location", models.CharField(blank=True, max_length=255, null=True)),
                ("duration_weeks", models.IntegerField(blank=True, null=True)),
                ("capacity", models.IntegerField(blank=True, null=True)),
                ("is_online", models.BooleanField()),
                ("zoom_link", models.TextField(blank=True, null=True)),
                ("is_active", models.BooleanField()),
                ("created_at", models.DateTimeField(auto_now_add=True)),
            ],
            options={
                "db_table": "classroom",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="InstructorProfile",
            fields=[
                (
                    "instructor_profile_id",
                    models.AutoField(primary_key=True, serialize=False),
                ),
                ("full_name", models.CharField(max_length=255)),
                ("staff_no", models.CharField(max_length=50, unique=True)),
            ],
            options={
                "db_table": "instructor_profile",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="StudentProfile",
            fields=[
                (
                    "student_profile_id",
                    models.AutoField(primary_key=True, serialize=False),
                ),
                ("first_name", models.CharField(blank=True, max_length=120, null=True)),
                ("last_name", models.CharField(blank=True, max_length=120, null=True)),
                ("titled", models.CharField(blank=True, max_length=40, null=True)),
                ("full_name", models.CharField(blank=True, max_length=255, null=True)),
                (
                    "student_no",
                    models.CharField(
                        default=api.models.generate_student_id,
                        max_length=50,
                        unique=True,
                    ),
                ),
                ("locked_at", models.DateTimeField(auto_now_add=True)),
            ],
            options={
                "db_table": "student_profile",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="User",
            fields=[
                ("user_id", models.AutoField(primary_key=True, serialize=False)),
                ("email", models.CharField(max_length=255, unique=True)),
                ("password_hash", models.CharField(max_length=255)),
                ("role", models.CharField(max_length=50)),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("is_active", models.BooleanField(default=True)),
            ],
            options={
                "db_table": "user",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="Course",
            fields=[
                (
                    "course_id",
                    models.CharField(
                        default=api.models.generate_custom_id,
                        max_length=6,
                        primary_key=True,
                        serialize=False,
                        unique=True,
                    ),
                ),
                ("title", models.CharField(max_length=255)),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("Active", "ACTIVE"),
                            ("Inactive", "INACTIVE"),
                            ("Draft", "DRAFT"),
                        ],
                        default="Active",
                        max_length=50,
                    ),
                ),
                ("credits", models.IntegerField(blank=True, null=True)),
                ("director", models.CharField(blank=True, max_length=50, null=True)),
                ("description", models.TextField(blank=True, null=True)),
                (
                    "owner_instructor",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.instructorprofile",
                    ),
                ),
            ],
            options={
                "db_table": "course",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="Lesson",
            fields=[
                (
                    "lesson_id",
                    models.CharField(max_length=32, primary_key=True, serialize=False),
                ),
                ("title", models.CharField(max_length=255)),
                ("description", models.TextField(blank=True, null=True)),
                ("objectives", models.TextField(blank=True, null=True)),
                (
                    "duration_weeks",
                    models.PositiveIntegerField(
                        blank=True,
                        choices=[(2, "2"), (3, "3"), (4, "4")],
                        default=4,
                        null=True,
                    ),
                ),
                ("credits", models.IntegerField()),
                (
                    "status",
                    models.CharField(
                        choices=[
                            ("Active", "ACTIVE"),
                            ("Inactive", "INACTIVE"),
                            ("Archived", "ARCHIVED"),
                        ],
                        default="Active",
                        max_length=50,
                    ),
                ),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "course",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING, to="api.course"
                    ),
                ),
                (
                    "created_by",
                    models.ForeignKey(
                        db_column="created_by",
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        related_name="lesson_created_by_set",
                        to="api.instructorprofile",
                    ),
                ),
                (
                    "designer",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.instructorprofile",
                    ),
                ),
            ],
            options={
                "db_table": "lesson",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="LessonAssignment",
            fields=[
                ("assignment_id", models.AutoField(primary_key=True, serialize=False)),
                ("title", models.CharField(max_length=255)),
                ("description", models.TextField(blank=True, null=True)),
                ("points", models.IntegerField(blank=True, null=True)),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "lesson",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING, to="api.lesson"
                    ),
                ),
            ],
            options={
                "db_table": "lesson_assignment",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="LessonClassroom",
            fields=[
                (
                    "lesson_classroom_id",
                    models.AutoField(primary_key=True, serialize=False),
                ),
                ("session_times_json", models.JSONField(blank=True, null=True)),
                ("linked_at", models.DateTimeField(auto_now_add=True)),
                (
                    "classroom",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.classroom",
                    ),
                ),
                (
                    "lesson",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING, to="api.lesson"
                    ),
                ),
            ],
            options={
                "db_table": "lesson_classroom",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="LessonReading",
            fields=[
                ("reading_id", models.AutoField(primary_key=True, serialize=False)),
                ("title", models.CharField(max_length=255)),
                ("url", models.TextField(blank=True, null=True)),
                ("created_at", models.DateTimeField(auto_now_add=True)),
                ("updated_at", models.DateTimeField(auto_now=True)),
                (
                    "lesson",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING, to="api.lesson"
                    ),
                ),
            ],
            options={
                "db_table": "lesson_reading",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="StudentAssignment",
            fields=[
                (
                    "student_assignment_id",
                    models.AutoField(primary_key=True, serialize=False),
                ),
                ("is_completed", models.BooleanField()),
                (
                    "assignment",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.lessonassignment",
                    ),
                ),
                (
                    "student",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.studentprofile",
                    ),
                ),
            ],
            options={
                "db_table": "student_assignment",
                "managed": True,
            },
        ),
        migrations.CreateModel(
            name="StudentReading",
            fields=[
                (
                    "student_reading_id",
                    models.AutoField(primary_key=True, serialize=False),
                ),
                ("is_completed", models.BooleanField()),
                (
                    "reading",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.lessonreading",
                    ),
                ),
                (
                    "student",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.studentprofile",
                    ),
                ),
            ],
            options={
                "db_table": "student_reading",
                "managed": True,
            },
        ),
        migrations.AddField(
            model_name="studentprofile",
            name="user",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.DO_NOTHING, to="api.user"
            ),
        ),
        migrations.AddField(
            model_name="instructorprofile",
            name="user",
            field=models.ForeignKey(
                on_delete=django.db.models.deletion.DO_NOTHING, to="api.user"
            ),
        ),
        migrations.CreateModel(
            name="LessonPrerequisite",
            fields=[
                ("prereq_id", models.AutoField(primary_key=True, serialize=False)),
                (
                    "lesson",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING, to="api.lesson"
                    ),
                ),
                (
                    "prereq_lesson",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        related_name="lessonprerequisite_prereq_lesson_set",
                        to="api.lesson",
                    ),
                ),
            ],
            options={
                "db_table": "lesson_prerequisite",
                "managed": True,
                "unique_together": {("lesson", "prereq_lesson")},
            },
        ),
        migrations.CreateModel(
            name="LessonEnrollment",
            fields=[
                (
                    "lesson_enrollment_id",
                    models.AutoField(primary_key=True, serialize=False),
                ),
                ("enrolled_at", models.DateTimeField(auto_now_add=True)),
                (
                    "lesson",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING, to="api.lesson"
                    ),
                ),
                (
                    "student",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.studentprofile",
                    ),
                ),
            ],
            options={
                "db_table": "lesson_enrollment",
                "managed": True,
                "unique_together": {("lesson", "student")},
            },
        ),
        migrations.CreateModel(
            name="Enrollment",
            fields=[
                ("enrollment_id", models.AutoField(primary_key=True, serialize=False)),
                ("enrolled_at", models.DateTimeField(auto_now_add=True)),
                (
                    "course",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING, to="api.course"
                    ),
                ),
                (
                    "student",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.studentprofile",
                    ),
                ),
            ],
            options={
                "db_table": "enrollment",
                "managed": True,
                "unique_together": {("student", "course")},
            },
        ),
        migrations.CreateModel(
            name="ClassroomEnrollment",
            fields=[
                (
                    "id",
                    models.BigAutoField(
                        auto_created=True,
                        primary_key=True,
                        serialize=False,
                        verbose_name="ID",
                    ),
                ),
                ("enrolled_at", models.DateTimeField(auto_now_add=True)),
                (
                    "classroom",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.classroom",
                    ),
                ),
                (
                    "student",
                    models.ForeignKey(
                        on_delete=django.db.models.deletion.DO_NOTHING,
                        to="api.studentprofile",
                    ),
                ),
            ],
            options={
                "db_table": "classroom_enrollment",
                "managed": True,
                "unique_together": {("classroom", "student")},
            },
        ),
    ]
