...
class StudentSerializer(serializers.ModelSerializer):
    #Represent target of relationships using primary keys
    course_ids = serializers.PrimaryKeyRelatedField(
        many=True, source="courses", queryset=Course.objects.all(), write_only=True, required=False
    )
    courses = CourseSerializer(many=True, read_only=True)
    class Meta:
        model = Student
        fields = ["student_profile_id", "full_name", "student_no", "courses", "course_ids"]
        read_only_fields = ["student_profile_id"] #autogenerated 

"""
    def create(self, validated_data):
        courses = validated_data.pop("courses", [])
        student = Student.objects.create(**validated_data) #unpacking 
        if courses:
            student.courses.set(courses) #replacing all courses
        return student #Creating a student 

    def update(self, instance, validated_data): #updating a student course
